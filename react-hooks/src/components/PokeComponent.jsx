...
const apiReducer = (state, action) => {
  switch (action.type) {
    ...
    case "FETCH_FAIL":
      ...
      return {
        loading: false,
        error: "There is no pokemon you need",
        pokemon: {},
      };
    default:
      console.log("Default");
...
function PokeComponent() {
  const [valueState, dispatchValue] = useReducer(searchReducer, searchState);
  const [fetchState, dispatchApi] = useReducer(apiReducer, apiState);

  // FetchAPi
  useEffect(() => {
    console.log("Mounting");
    axios
      .get(`https://pokeapi.co/api/v2/pokemon/${valueState.button}`)
      .then((res) => {
        dispatchApi({ type: "FETCH_SUCCESS", payload: res.data });
      })
      .catch((err) => {
        dispatchApi({ type: "FETCH_FAIL" });
      });
  }, [valueState.button]);
    ...
    <>
      <div>
        <input
          ...
          onChange={(e) => {
            dispatchValue({ type: "input", payload: e.target.value });
          }}
        />
        <button
          onClick={() => {
            dispatchValue({ type: "button", payload: valueState.search });
          }}
        >
          Search Pokemon
        </button>
